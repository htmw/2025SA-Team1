       ...
       for user in users:
        ...
        if(user_id in user_ids or len(user_ids) == 0):
            for key in user.val()[user_id].keys():
                if(not isinstance(user.val()[user_id][key], dict) and user.val()[user_id][key].lower() == tags[0].lower()):
                    filter_users.append(user_id)
                elif(isinstance(user.val()[user_id][key], dict)):
                    for subkey in user.val()[user_id][key]:
                        for element in user.val()[user_id][key][subkey]:
                            if(element.lower() == tags[0].lower()):
                                filter_users.append(user_id)
       tags.remove(tags[0])
